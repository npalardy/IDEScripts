...
// KeyboardShortcut=CMD-OPT-SHIFT-U

Const IDESCript = True
Const debugThis = True

DoInsertProgramUnusedForParams()

Sub DoInsertProgramUnusedForParams()
	...
	// Print( "CLIPBOARD " + EndOfLine +  clipboard )
	...
	// Print( "CLEANED !" + EndOfLine + content )
	...
	ElseIf LanguageUtils.CrackEventDeclaration(content, scope, subFunc, methodName, params, returntype) Then
		// Print("Its an event")
	Else
		// Print("Its NOT a method or event")
		Return
	End If
	...
	For Each l As LanguageUtils.LocalVariable In parms
		...
		to_insert.append "#pragma unused " + l.name
